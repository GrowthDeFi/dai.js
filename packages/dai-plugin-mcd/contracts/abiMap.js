// not run through babel, so we must use an ES5-safe export
module.exports = {
  REP: require('./abis/REP.json'),
  ZRX: require('./abis/ZRX.json'),
  OMG: require('./abis/OMG.json'),
  BAT: require('./abis/BAT.json'),
  DGD: require('./abis/DGD.json'),
  KNC: require('./abis/KNC.json'),
  GNT: require('./abis/GNT.json'),
  SAI: require('./abis/DSToken.json'),
  CDP_MANAGER: require('./abis/DssCdpManager.json'),
  BNB: require('./abis/WETH9_.json'),
  GET_CDPS: require('./abis/GetCdps.json'),
  MCD_ADM: require('./abis/DSRoles.json'),
  MCD_CAT: require('./abis/Cat.json'),
  OLD_MCD_CAT: require('./abis/Cat.json'),
  MCD_DAI: require('./abis/DSToken.json'),
  MCD_DAI_GUARD: require('./abis/DSGuard.json'),
  MCD_DEPLOY: require('./abis/DssDeploy.json'),
  MCD_FLAP: require('./abis/Flapper.json'),
  'MCD_FLIP_*': require('./abis/Flipper.json'),
  MCD_FLOP: require('./abis/Flopper.json'),
  MCD_GOV: require('./abis/DSToken.json'),
  'MCD_JOIN_*': require('./abis/GemJoin.json'),
  MCD_JOIN_OMG_A: require('./abis/GemJoin2.json'),
  MCD_JOIN_DGD_A: require('./abis/GemJoin2.json'),
  MCD_JOIN_GNT_A: require('./abis/GemJoin4.json'),
  MCD_JOIN_DAI: require('./abis/DaiJoin.json'),
  MCD_JUG: require('./abis/Jug.json'),
  MCD_END: require('./abis/End.json'),
  MCD_ESM: require('./abis/ESM.json'),
  MCD_PAUSE: require('./abis/DSPause.json'),
  MCD_PAUSE_PROXY: require('./abis/Plan.json'),
  MCD_POT: require('./abis/Pot.json'),
  MCD_SPOT: require('./abis/Spotter.json'),
  MCD_VAT: require('./abis/Vat.json'),
  MCD_VOW: require('./abis/Vow.json'),
  MULTICALL: require('./abis/MultiCall.json'),
  'PIP_*': require('./abis/DSValue.json'),
  PROXY_ACTIONS: require('./abis/DssProxyActions.json'),
  PROXY_ACTIONS_DSR: require('./abis/DssProxyActionsDsr.json'),
  MIGRATION: require('./abis/ScdMcdMigration.json'),
  MIGRATION_PROXY_ACTIONS: require('./abis/MigrationProxyActions.json'),
  USDC: require('./abis/DSToken.json'),
  WBTC: require('./abis/WBTC.json'),
  TUSD: require('./abis/DSToken.json'),
  MANA: require('./abis/DSToken.json'),
  USDT: require('./abis/DSToken.json'),
  PAXUSD: require('./abis/DSToken.json'),
  COMP: require('./abis/DSToken.json'),
  LRC: require('./abis/DSToken.json'),
  LINK: require('./abis/DSToken.json'),
  YFI: require('./abis/DSToken.json'),
  BAL: require('./abis/DSToken.json'),
  GUSD: require('./abis/GUSD.json'),
  UNI: require('./abis/DSToken.json'),
  RENBTC: require('./abis/DSToken.json'),
  AAVE: require('./abis/DSToken.json'),
  ETH: require('./abis/DSToken.json'),
  BUSD: require('./abis/DSToken.json'),
  BTCB: require('./abis/DSToken.json'),
  CAKE: require('./abis/DSToken.json'),
  BANANA: require('./abis/DSToken.json'),
  UNIV2DAIETH: require('./abis/DSToken.json'),
  UNIV2WBTCETH: require('./abis/DSToken.json'),
  UNIV2USDCETH: require('./abis/DSToken.json'),
  UNIV2DAIUSDC: require('./abis/DSToken.json'),
  UNIV2ETHUSDT: require('./abis/DSToken.json'),
  UNIV2LINKETH: require('./abis/DSToken.json'),
  UNIV2UNIETH: require('./abis/DSToken.json'),
  UNIV2WBTCDAI: require('./abis/DSToken.json'),
  UNIV2AAVEETH: require('./abis/DSToken.json'),
  UNIV2DAIUSDT: require('./abis/DSToken.json'),
  PCSBNBCAKE: require('./abis/DSToken.json'),
  PCSBNBBUSD: require('./abis/DSToken.json'),
  PCSBNBETH: require('./abis/DSToken.json'),
  PCSBNBBTCB: require('./abis/DSToken.json'),
  PCSBUSDUSDC: require('./abis/DSToken.json'),
  PCSBUSDBTCB: require('./abis/DSToken.json'),
  PCSBUSDCAKE: require('./abis/DSToken.json'),
  PCSETHBTCB: require('./abis/DSToken.json'),
  PCSETHUSDC: require('./abis/DSToken.json'),
  APEMORBUSD: require('./abis/DSToken.json'),
  STKCAKE: require('./abis/DSToken.json'),
  STKBANANA: require('./abis/DSToken.json'),
  STKPCSBNBCAKE: require('./abis/DSToken.json'),
  STKPCSBNBBUSD: require('./abis/DSToken.json'),
  STKPCSBNBETH: require('./abis/DSToken.json'),
  STKPCSBNBBTCB: require('./abis/DSToken.json'),
  STKPCSBUSDUSDC: require('./abis/DSToken.json'),
  STKPCSBUSDBTCB: require('./abis/DSToken.json'),
  STKPCSBUSDCAKE: require('./abis/DSToken.json'),
  STKPCSETHBTCB: require('./abis/DSToken.json'),
  STKPCSETHUSDC: require('./abis/DSToken.json'),
  STKAPEMORBUSD: require('./abis/DSToken.json'),
  AVAX: require('./abis/WETH9_.json'),
  WETH:  require('./abis/DSToken.json'),
  DAI: require('./abis/DSToken.json'),
  MIM: require('./abis/DSToken.json'),
  JOE: require('./abis/DSToken.json'),
  XJOE: require('./abis/DSToken.json'),
  JAVAX: require('./abis/DSToken.json'),
  JWETH: require('./abis/DSToken.json'),
  JWBTC: require('./abis/DSToken.json'),
  JLINK: require('./abis/DSToken.json'),
  TDJAVAXJOE: require('./abis/DSToken.json'),
  TDJAVAXWETH: require('./abis/DSToken.json'),
  TDJAVAXWBTC: require('./abis/DSToken.json'),
  TDJAVAXDAI: require('./abis/DSToken.json'),
  TDJAVAXUSDC: require('./abis/DSToken.json'),
  TDJAVAXUSDT: require('./abis/DSToken.json'),
  TDJAVAXLINK: require('./abis/DSToken.json'),
  TDJAVAXMIM: require('./abis/DSToken.json'),
  TDJUSDCJOE: require('./abis/DSToken.json'),
  TDJUSDTJOE: require('./abis/DSToken.json'),
  STKXJOE: require('./abis/DSToken.json'),
  STKJAVAX: require('./abis/DSToken.json'),
  STKJWETH: require('./abis/DSToken.json'),
  STKJWBTC: require('./abis/DSToken.json'),
  STKJLINK: require('./abis/DSToken.json'),
  STKTDJAVAXJOE: require('./abis/DSToken.json'),
  STKTDJAVAXWETH: require('./abis/DSToken.json'),
  STKTDJAVAXWBTC: require('./abis/DSToken.json'),
  STKTDJAVAXDAI: require('./abis/DSToken.json'),
  STKTDJAVAXUSDC: require('./abis/DSToken.json'),
  STKTDJAVAXUSDT: require('./abis/DSToken.json'),
  STKTDJAVAXLINK: require('./abis/DSToken.json'),
  STKTDJAVAXMIM: require('./abis/DSToken.json'),
  STKTDJUSDCJOE: require('./abis/DSToken.json'),
  STKTDJUSDTJOE: require('./abis/DSToken.json')
};
